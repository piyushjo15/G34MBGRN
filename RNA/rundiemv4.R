## This script uses Gene and GeneFull counts from STAR aligner to generate a combined # This version includes SoupX on uncorrected counts. 
...
DIR="~/DATA/kons_analysis/indana/MBnew/"
...
# 
## not run -----
# ##mclust to find cells based on fraction plot
# cl <- Mclust(mdfr2[,c("frac")], G=2)
# #finding'class' of cells
# d <- table(factor(cl$classification))
# k <- as.integer(which(d==min(d)))
# #knee point as posisble number of cells
# 
# #finding cells from probability (z)
# keep <- cl$z[,k]>=0.95
# table(keep)
# kp <- as.integer(table(keep)[2])
#
#find similar cells----
...
#drop_data <- droplet_data(mdt)
# table(drop_data[,"Cluster"])
# tapply(drop_data[,"n_genes"],drop_data[,"Cluster"], mean)
# tapply(drop_data[,"pct.mt"], drop_data[,"Cluster"], mean)
...
#head(de_genes)
mdt <- call_targets(mdt,thresh_score = .5)
