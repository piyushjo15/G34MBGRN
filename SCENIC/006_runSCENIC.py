...
#load TSV file
RNA_Sample = Sample.replace("MSM","MSN")

file_path = projDir+'TF2G/cor'+ RNA_Sample +'.tsv'
tf2g = pd.read_csv(file_path, sep='\t') ##this is the output of 004b_runpyscenic.sh
#remvoe NA from 'rho' column
tf2g_cleaned = tf2g.dropna(subset=['rho'])
#save as TSV
output_file_path = projDir+'TF2G/cor'+ RNA_Sample +'_cleaned.tsv'
tf2g_cleaned.to_csv(output_file_path, sep='\t', index=False)

from scenicplus.TF_to_gene import *
load_TF2G_adj_from_file(scplus_obj,
                        f_adj = projDir+'TF2G/cor'+ RNA_Sample +'_cleaned.tsv',
                        inplace = True,
                        key= 'TF2G_adj')
