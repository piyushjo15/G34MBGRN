...
Sample <- commandArgs(TRUE) ##SA194
msn <- "SN407"

DIR = "SCENIC/scenicplus/"
#read metadata
...
head(bed)
write.table(bed,paste0("metadata/",Sample,"_enhancersv2.bed"),quote = FALSE, row.names = FALSE, col.names = FALSE, sep ="\t")

##extracting links
REGS <- GRNS2[,c("TF","chr","start","end","ID","Gene")]
save(REGS,file = paste0("metadata/",Sample,"_TF2R2G.RData"))
