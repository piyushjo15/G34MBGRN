## This script fixes missing links in SCENIC= identified GRNS
## SCENIC+ links certain enhancers to a target genes, and also identifies TF
## binding sites in them. These enhancers linked to more than one genes, are 
## somehow assigned to TF regulation for one set of genes while not for others
## This script fixes these missing links, based on a specific cut-off between
## the TF and the missing downstreat target.
...
DIR = "/omics/odcf/analysis/OE0290_projects/mb_10x/ATACana/SCENIC/scenicplus/"
#read metadata
setwd(paste0(DIR, Sample, "/output/"))

df <- read.csv(paste0("metadata/",Sample,"_eRegulon_metadata.csv"), sep = ";")
...
write.table(reg_adj, file=paste0("metadata/",Sample,"_enhancers.bed"), 
...
head(GRNS)
#get a region to gene matrix
setwd("/omics/odcf/analysis/OE0290_projects/mb_10x/ATACana/SCENIC/TF2G/")
TF2G <- read.delim(paste0("cor",msn,"_cleaned.tsv")) ### fix the file loc
setwd(paste0(DIR, Sample, "/output/"))
...
tail(GRNS2)
write.table(GRNS2,paste0("metadata/",Sample,"_eRegulons_fix.txt"),quote = FALSE, row.names = FALSE, sep ="\t")
...
rm(x)
save(regs, file = paste0("metadata/",msn,"_gmt.RData"))
#make a bed file
...
head(bed)
write.table(bed,paste0("metadata/",msn,"_enhancersv2.bed"),quote = FALSE, row.names = FALSE, col.names = FALSE, sep ="\t")

##extracting links
REGS <- GRNS2[,c("TF","chr","start","end","ID","Gene")]
save(REGS,file = paste0("metadata/",msn,"_TF2R2G.RData"))
